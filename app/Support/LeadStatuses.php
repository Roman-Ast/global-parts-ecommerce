...
class LeadStatuses
{
     ...
     * своих/коллег/поставщиков, попавших в лиды по ошибке (в отличие от
     * EXCLUDED_PHONES в WhatsAppWebhookController — те исключены заранее,
     * до появления лида; сюда Роман переносит вручную УЖЕ существующий
     * лид, когда понимает постфактум, что это не клиент). В отличие от
     * SPAM_STATUS — это НАСТОЯЩАЯ колонка на доске (see all(), сразу после
     * 'new'), не просто счётчик-корзина: Роман хочет видеть, кто туда
     * попал. Специальное поведение — WhatsAppWebhookController перестаёт
     * сохранять НОВЫЕ сообщения по номеру лида в этом статусе (см. докблок
     * там же); уже сохранённая история не трогается.
     */
