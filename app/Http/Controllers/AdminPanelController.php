...
class AdminPanelController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // План поднят с ~9.3М/мес (300 000/день, порог "отличного дня"
        // 390 000 — то же соотношение ×1.3) до 11М/мес по просьбе Романа
        // 2026-09-18. Считается от количества дней в текущем учётном периоде
        // (8-е числа по 7-е следующего месяца, см. $startForDailyStats выше),
        // а не фиксированным числом в день — раньше 300 000/день был подобран
        // вручную под ~31-дневный период и слегка плавал по месяцам.
        // ВАЖНО: раскраска точек графика считается ЖИВЬЁМ при каждой
        // загрузке страницы по ТЕКУЩЕЙ константе — она применяется и к
        // прошлым периодам тоже (плана "как было на самом деле тогда"
        // нигде не хранится). Значит вся статистика ДО 2026-09-18,
        // подсчитанная по старому плану (~9.3М), при пересчёте сейчас
        // будет сравниваться с новым 11М планом — красные/жёлтые дни в
        // старых периодах не обязательно означают провал по факту
        // тогдашней цели, они просто теперь меряются другой линейкой.
        $daysInPeriod = $startForDailyStats->copy()->toPeriod($endForDailyStats)->count();
        $planPerDay = 11_000_000 / $daysInPeriod;
