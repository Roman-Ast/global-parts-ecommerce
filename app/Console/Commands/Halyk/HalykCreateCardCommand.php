...
class HalykCreateCardCommand extends Command
{
     ...
     * СТРОКА, но смысл этой строки разный по типу поля: для STRING —
     * свободный текст ("Упаковка картонная"/"true"/"5" из их доки), для
     * ENUM — classAttrValueId выбранного варианта (см. matchEnumOption(),
     * исправлено 2026-08-28 — раньше слали имя варианта, оно молча не
     * сохранялось). Если обязательный атрибут не удаётся заполнить —
     ...
     * см. докблок flattenCharacteristics). Возвращает classAttrValueId
     * (СТРОКОЙ) найденного варианта, либо null, если ни одно из наших
     * значений не совпало ни с одним вариантом Halyk.
     *
     * ИСПРАВЛЕНО 2026-08-28 — раньше возвращали ИМЯ варианта ("Передние"),
     * не classAttrValueId ("20319"). Так было сделано по примеру из их
     * документации (там value для ENUM показан как читаемая строка), и
     * карточки с этим форматом реально проходили модерацию — но, как
     * объяснил их партнёрский менеджер по нашему обращению про баг с
     * характеристиками, именно из-за этого формата значение ENUM-поля
     * молча НЕ сохранялось в карточку ("эти данные не уходят в контент") —
     * карточка создавалась и проходила модерацию, но конкретно это поле
     * оставалось пустым на живой странице товара, о чём нам знать было
     * неоткуда (API на это не жаловался).
     */
        ...
        foreach ($ourValues as $ourValue) {
            ...
            foreach ($options as $opt) {
                ...
                if ($optLower === $ourLower || str_contains($ourLower, $optLower) || str_contains($optLower, $ourLower)) {
                    return (string) ($opt['classAttrValueId'] ?? $optName);
                }
            }
        }
