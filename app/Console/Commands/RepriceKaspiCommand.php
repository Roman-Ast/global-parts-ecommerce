...
class RepriceKaspiCommand extends Command
{
    ...
    // Откачено обратно на 30/20/12/8 (2026-09-23) — подъём до 33/23/15/11
    // (2026-08-27) поднимал маржу, но живым разбором Kaspi-аналитики за
    // 4 месяца (55427 строк кликов/продаж, см. переписку с Романом
    // 2026-09-23) выяснилось: доминирующий фактор конверсии — "Доля в
    // карточке" (доля кликов на общей Kaspi-карточке, где мы один из
    // продавцов), а не абсолютный уровень цены — разница между 0-24% и
    // 100% доли ~3000x по конверсии. Поднятый пол маржи мешал репрайсеру
    // перебивать конкурента с доставкой "завтра" на 1801 позиции (из
    // 73861, apples-to-apples замер --dry-run на свежих прайсах
    // 2026-09-23) — они стояли ВЫШЕ конкурента
    // (сценарий min_margin_dumping = пол маржи, но пол оказывается выше
    // цены конкурента) вместо того чтобы реально с ним конкурировать
    // (beat_tomorrow_competitor). Откат не создаёт риска: 🔴 Риск
    // (маржа в минус) = 0 в обоих вариантах шкалы, это просто более
    // тонкая подушка над безопасным полом, не уход в убыток.
    ...
    const SUPPLIER_FIXED_COST_EXCLUDE_ARTICLES = [
        ...
        // SAT передние — закуп 9250 за 1 диск, kaspi_qty у карточки и так
        // уже честно =1 (в отличие от ST4351232280 выше, где было =2).
        // Само число cost сегодня не меняется (9250*1 = 9250, что и без
        // исключения), но без исключения расчёт был бы "случайно верным"
        // только пока kaspi_qty=1 — если карточка Kaspi когда-нибудь
        // поменяется на =2 (как у похожих артикулов), фиксированная
        // логика молча продолжила бы держать cost=9250 вместо честных
        // 18500, и позиция снова ушла бы в минус тем же путём, что и
        // ST4351232280.
        'ST4351233130',
    ];
