...
<div class="p-6 bg-slate-100 min-h-screen" wire:poll.5s.visible="pollTick">
                    ...
                    <div class="grid grid-cols-{{ count($info['sub']) }} gap-3 h-full min-h-0">
                        @foreach($info['sub'] as $subKey => $subTitle)
                            ...
                            {{-- wire:key отсутствовал на этой обёртке (в отличие от всех
                                 ...
                                 ключа Livewire на каждый wire:poll.5s не может надёжно
    ...
    <style>
        ...
        @keyframes card-pulse-border {
            0%, 100% { outline: 2px solid rgba(244,63,94,.6); outline-offset: 0px; }
    ...
    <script>
        ...
        // wire:poll.5s (шапка всего компонента), то есть каждые 5 секунд,
        ...
        // Плюс это просто лишняя работа каждые 5 секунд на ровном месте.
        //
        ...
        function initKanban() {
            document.querySelectorAll('.kanban-column').forEach(el => {
                ...
                new Sortable(el, {
                    ...
                    // это разошлось с wire:poll.5s, который продолжает морфить карточки
                    // каждые 5 секунд даже во время попытки перетаскивания (kanbanDragActive
        ...
        // суммы непрочитанных с предыдущим тиком wire:poll.5s), сюда
